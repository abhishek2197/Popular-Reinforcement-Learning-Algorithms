...
class ActorNetwork(nn.Module):
	def __init__(self, stdim, acdim, aclim):
		...
		self.acdim = acdim
		self.aclim = aclim
		'''self.bn0 = nn.BatchNorm1d(self.stdim)
		...
		self.finallay.weight.data.mul_(0.1)
		self.finallay.bias.data.mul_(0.1)'''
		self.firstlay = nn.Linear(self.stdim, 256)
...
class CriticNetwork(nn.Module):
	def __init__(self, stdim, acdim):
		...
		self.finallay_state_action.weight.data.mul_(0.1)
		self.finallay_state_action.bias.data.mul_(0.1)'''
